...
async function deleteAlbum(id){if(!confirm("Az album és minden képe törlődik. Biztos?"))return;for(const p of state.photos.filter(x=>x.albumId===id)){await removeStored(p.storagePath);await deleteDoc(doc(db,"photos",p.id))}const a=state.albums.find(x=>x.id===id);await removeStored(a.coverPath);await deleteDoc(doc(db,"albums",id));toast("Album törölve");loadAlbums()}
$("saveAlbum").onclick=async()=>{try{const id=$("albumId").value,old=id?state.albums.find(x=>x.id===id):{};const img=await uploadImage($("albumCover").files[0],"album-covers");const data={name:$("albumName").value.trim(),date:$("albumDate").value,facebook:$("albumFacebook").value.trim(),description:$("albumDescription").value.trim(),published:$("albumPublished").value==="true",coverUrl:img?.url||old.coverUrl||"",coverPath:img?.path||old.coverPath||"",updatedAt:new Date().toISOString()};if(img&&old.coverPath)await removeStored(old.coverPath);if(id)await updateDoc(doc(db,"albums",id),data);else await addDoc(collection(db,"albums"),{...data,createdAt:new Date().toISOString()});resetAlbum();toast("Album mentve");loadAlbums()}catch(e){toast(e.message,"error")}}
...
async function delSimple(col,id,reload){if(confirm("Törlöd?")){await deleteDoc(doc(db,col,id));toast("Törölve");reload()}}
...
async function deleteSponsor(id){if(!confirm("Törlöd?"))return;const x=state.sponsors.find(v=>v.id===id);await removeStored(x.logoPath);await deleteDoc(doc(db,"sponsors",id));loadSponsors()}
...
async function saveSimple(col,prefix,fields,reload){try{const id=$(`${prefix}Id`).value,data={updatedAt:new Date().toISOString()};fields.forEach(f=>{let v=$(`${prefix}${f[0].toUpperCase()+f.slice(1)}`).value;if(v==="true"||v==="false")v=v==="true";data[f]=v});if(id)await updateDoc(doc(db,col,id),data);else await addDoc(collection(db,col),{...data,createdAt:new Date().toISOString()});resetSimple(prefix,fields);toast("Mentve");reload()}catch(e){toast(e.message,"error")}}
