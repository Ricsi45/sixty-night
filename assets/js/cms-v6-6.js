...
import { firebaseConfig, ADMIN_EMAIL } from "./firebase-config.js?v=6.6.0";
import { LEGACY_CONTENT } from "./legacy-content.js?v=6.6.0";
...
const views=["dashboard","homepage","events","performers","albums","videos","sponsors","tickets","pages","settings"];
...
async function refreshAll(){await Promise.all([loadEvents(),loadPerformers(),loadAlbums(),loadVideos(),loadSponsors(),loadTickets(),loadPages(),loadSettings()]);await loadHomepageEditor();updateStats()}
...
$("previewHomepage").onclick=async()=>{const previewWindow=window.open("about:blank","_blank");const ok=await hpPersist("draft");if(ok){const url=`/index.html?homepagePreview=1&v=660`;if(previewWindow)previewWindow.location.href=url;else window.open(url,"_blank")}else if(previewWindow)previewWindow.close()};
