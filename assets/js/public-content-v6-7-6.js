...
function logicalKey(collectionName, item) {
  if (collectionName === "photos") return String(item.id || "");
...
async function readPublishedCollectionState(collectionName) {
  ...
  const inactiveKeys = new Set();
  groups.forEach((items, key) => {
    const visible = items.filter((item) => item.deleted !== true && item.hidden !== true);
    if (visible.length) active.push(mergeGroup(visible));
    else inactiveKeys.add(key);
  });
  return { active, inactiveKeys };
...
function albumPhotos(album, photos) {
  ...
  return photos
    .filter((photo) => ids.has(photo.albumId) && photo.deleted !== true && photo.hidden !== true)
    .sort(compareAlbumPhotos);
}
...
function albumDateValue(album) {
  const value = Date.parse(album.date || album.createdAt || "");
  return Number.isFinite(value) ? value : 0;
}

function sortAlbums(albums) {
  return [...albums].sort((a, b) => albumDateValue(b) - albumDateValue(a) || String(a.name || "").localeCompare(String(b.name || ""), "hu"));
}
...
function openAlbum(album, photos) {
  ...
  modal.innerHTML = `<div class="album-modal-inner"><div class="album-modal-head"><div><div class="eyebrow">SIXTY NIGHT PARTY</div><h2>${escapeHtml(album.name || "Album")}</h2><p>${escapeHtml(album.date || "")}</p></div><button class="album-close" type="button" aria-label="Bezárás">×</button></div><div class="album-grid">${photos.length ? photos.map((photo, index) => `<button class="album-photo-button" type="button" data-album-photo-index="${index}"><img loading="lazy" src="${escapeHtml(photo.url || "")}" alt="${escapeHtml(album.name || "Album")} – ${index + 1}. kép"></button>`).join("") : '<div class="empty-album">Nincs kép ebben az albumban.</div>'}</div></div>`;
  modal.classList.add("open");
  const close = () => modal.classList.remove("open");
  modal.querySelector(".album-close")?.addEventListener("click", close);
  modal.addEventListener("click", (event) => { if (event.target === modal) close(); }, { once: true });
  modal.querySelectorAll("[data-album-photo-index]").forEach((button) => {
    button.addEventListener("click", () => {
      const photo = photos[Number(button.dataset.albumPhotoIndex)];
      if (!photo?.url) return;
      if (typeof window.openPhotoLightbox === "function") window.openPhotoLightbox(photo.url);
      else window.open(photo.url, "_blank", "noopener");
    });
  });
}
...
function albumCardHtml(album, photos) {
  ...
  const cover = albumCover(album, photos);
  return `<article class="photo-album-card" data-cms-generated="album" data-album-id="${escapeHtml(album.id)}"><div class="photo-cover"${cover ? ` style="background-image:url('${safeCssUrl(cover)}')"` : ""}></div><div class="photo-album-body"><div class="eyebrow">FOTÓALBUM${album.date ? ` • ${escapeHtml(album.date)}` : ""}</div><h3>${escapeHtml(album.name || "Album")}</h3><p>${escapeHtml(album.description || "")}</p><div class="album-actions"><button class="btn primary" data-open-cms-album="${escapeHtml(album.id)}">Webes galéria (${items.length})</button>${album.facebook ? `<a class="btn ghost" href="${escapeHtml(album.facebook)}" target="_blank" rel="noopener">Facebook album</a>` : ""}</div></div></article>`;
}

function matchingAlbumCard(album, cards, titleMap) {
  const sourceIds = albumSourceIds(album);
  const byId = cards.find((card) => sourceIds.has(card.dataset.albumId));
  return byId || matchingCard("albums", album, titleMap);
}

async function syncAlbums() {
  if (currentFile() !== "fotok.html" && !document.getElementById("firebaseAlbums")) return;
  const root = document.getElementById("firebaseAlbums") || document.querySelector(".photo-albums");
  if (!root) return;

  // A beépített albumkártyák Firebase-hiba esetén is dátum szerint maradnak.
  const staticCards = [...root.querySelectorAll('.photo-album-card:not([data-cms-generated="album"])')];
  staticCards
    .sort((a, b) => Date.parse(b.dataset.albumDate || "") - Date.parse(a.dataset.albumDate || ""))
    .forEach((card) => root.appendChild(card));

  const [{ active, inactiveKeys }, photoState] = await Promise.all([
    readPublishedCollectionState("albums"),
    readCollectionState("photos")
  ]);
  const photos = photoState.active;
  const albums = sortAlbums(active);
  const map = staticCardMap(staticCards, ["h3"], "albums");
  // Ha a Firebase-ben már vannak albumok, csak a ténylegesen publikusak maradnak láthatók.
  if (albums.length) staticCards.forEach((card) => { card.hidden = true; });
  hideInactiveStaticCards("albums", inactiveKeys, map);
  const matchedIds = new Set();
  const matchedCards = new Map();

  albums.forEach((album) => {
    const card = matchingAlbumCard(album, staticCards, map);
    if (!card) return;
    showCard(card);
    card.dataset.albumId = album.id;
    card.dataset.albumDate = album.date || "";
    matchedIds.add(album.id);
    matchedCards.set(album.id, card);
    const items = albumPhotos(album, photos);
    ...
    if (date && album.date) date.textContent = album.date;
    const facebook = [...card.querySelectorAll("a")].find((link) => /facebook/i.test(link.textContent));
    if (facebook) {
      if (album.facebook) { facebook.href = album.facebook; facebook.hidden = false; }
      else facebook.hidden = true;
    }
  ...
  const unmatched = albums.filter((album) => !matchedIds.has(album.id));
  renderGeneratedItems(root, "album", unmatched.map((album) => albumCardHtml(album, photos)).join(""));
  const generatedCards = new Map();
  root.querySelectorAll('[data-cms-generated="album"]').forEach((card) => {
    generatedCards.set(card.dataset.albumId, card);
  });
  ...
  // Minden publikus album – a régi kártyák és az új CMS-albumok is – egyetlen időrendbe kerül.
  albums.forEach((album) => {
    const card = matchedCards.get(album.id) || generatedCards.get(album.id);
...
const EXISTING_PAGE_DEFAULTS = {
  "index.html": { heroTitle: "SIXTY NIGHT PARTY – AZ ÉJSZAKA A TIÉD." },
  "esemenyek.html": { heroTitle: "KORÁBBI ESEMÉNYEINK" },
  "fellepok.html": { heroTitle: "EDDIGI FELLÉPŐINK" },
  "fotok.html": { heroTitle: "FOTÓK" },
  "videotar.html": { heroTitle: "NÉZD VISSZA A BULIKAT" },
  "rolunk.html": { heroTitle: "RÓLUNK" },
  "helyszin.html": { heroTitle: "SIXTY NIGHT PARTY PARK" },
  "vip.html": { heroTitle: "VIP CLUB" },
  "kapcsolat.html": { heroTitle: "ÍRJ NEKÜNK" },
  "mulat-hatvan.html": { heroTitle: "MULAT HATVAN" },
  "uv-cirkusz.html": { heroTitle: "UV CIRKUSZ 2.0" },
  "holi-jungle.html": { heroTitle: "HOLI JUNGLE" }
};

function publicFacingText(value, fallback = "") {
  const text = String(value || "").trim();
  if (!text) return fallback;
  if (/adminból\s+kezelt|admin\s+előnézet|weboldalon\s+kezelt/i.test(text)) return fallback;
  return text;
}
...
async function syncExistingPage() {
  ...
  const defaults = EXISTING_PAGE_DEFAULTS[file] || {};
  const page = {
    ...storedPage,
    heroTitle: publicFacingText(storedPage.heroTitle || storedPage.title, defaults.heroTitle || storedPage.title || ""),
    title: publicFacingText(storedPage.title, defaults.heroTitle || ""),
    intro: publicFacingText(storedPage.intro, "")
  };
  ...
  if (page.replaceMain === true && !isHomepage) {
...
async function boot() {
  const file = currentFile();
  // A Főoldal szöveges extra tartalma továbbra is az Oldalak modulból érkezik,
  // de a hero és a kiemelt esemény felületét a külön V6.6 főoldal-modul kezeli.
