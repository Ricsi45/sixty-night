...
import { firebaseConfig, ADMIN_EMAIL } from "./firebase-config.js?v=6.7.0";
import { LEGACY_CONTENT } from "./legacy-content.js?v=6.7.0";
...
async function loadTickets(){
  ...
  $("ticketsList").innerHTML=state.tickets.map(x=>{
    ...
    return itemHtml("",x.name,`${x.eventName||""}${x.eventName?" • ":""}${price||""} • ${mode}`,x.id,"ticket");
  }).join("")||"<p>Nincs jegy.</p>";
...
function resetTicket(){
  ["ticketId","ticketName","ticketPrice","ticketPriceValue","ticketUrl"].forEach(id=>$(id).value="");$("ticketEventId").value="";$("ticketActive").value="true";$("ticketOrderEnabled").value="false";$("ticketMaxPerOrder").value="6";
}
$("saveTicket").onclick=async()=>{try{
  ...
  const old=state.tickets.find(item=>item.id===id)||{};const data={name,eventId:event?.id||"",eventName:event?.name||"",eventDate:event?.date||"",eventLocation:event?.location||"Sixty Night Party Park • Hatvan",price:$("ticketPrice").value.trim(),priceValue, url:$("ticketUrl").value.trim(),active:$("ticketActive").value==="true",orderEnabled,maxPerOrder:Math.max(1,Math.min(20,Number($("ticketMaxPerOrder").value||6))),updatedAt:new Date().toISOString()};
  const targetId=id||stableDocId(name,event?.id||"");await setDoc(doc(db,"tickets",targetId),{...data,createdAt:old.createdAt||new Date().toISOString(),deleted:false,hidden:false},{merge:true});resetTicket();toast("Jegytípus mentve");await loadTickets();
}catch(e){console.error(e);toast(e.message||String(e),"error")}};
...
function openOrder(id){
  ...
  const cards=[["Rendelés",order.orderNumber],["Állapot",ORDER_STATUS[order.status]||order.status],["Beérkezett",huDate(order.createdAt||order.clientCreatedAt)],["Vásárló",order.buyerName],["E-mail",order.email],["Telefon",order.phone],["Esemény",order.eventName],["Jegytípus",`${order.ticketName||""} × ${Number(order.quantity||1)}`],["Összeg",money(order.total)],["Számlázási név",order.billingName],["Számlázási cím",`${order.billingZip||""} ${order.billingCity||""}, ${order.billingAddress||""}`],["Adószám",order.taxNumber||"—"],["Megjegyzés",order.note||"—","full"],["Jegykódok",Array.isArray(order.ticketCodes)&&order.ticketCodes.length?order.ticketCodes.join("\n"):"Még nincs generálva","full"]];
...
function randomTicketCode(){const bytes=new Uint8Array(18);crypto.getRandomValues(bytes);return [...bytes].map(value=>value.toString(36).padStart(2,"0")).join("").slice(0,28).toUpperCase()}
async function ensureTicketCodes(order){
  const quantity=Math.max(1,Math.min(20,Number(order.quantity||1)));const codes=Array.isArray(order.ticketCodes)?[...order.ticketCodes]:[];
  while(codes.length<quantity){const code=randomTicketCode();codes.push(code);await setDoc(doc(db,"issuedTickets",code),{code,orderId:order.id,orderNumber:order.orderNumber||order.id,eventName:order.eventName||"",eventDate:order.eventDate||"",ticketName:order.ticketName||"Belépőjegy",status:"valid",createdAt:serverTimestamp()},{merge:false})}
  if(codes.length>quantity)codes.length=quantity;
  await setDoc(doc(db,"ticketOrders",order.id),{ticketCodes:codes,status:"ticket_generated",ticketGeneratedAt:serverTimestamp(),updatedAt:serverTimestamp()},{merge:true});order.ticketCodes=codes;order.status="ticket_generated";return codes;
}
...
async function makeQrCanvas(text){const holder=$("ticketQrRender");holder.innerHTML="";new window.QRCode(holder,{text,width:300,height:300,colorDark:"#050509",colorLight:"#ffffff",correctLevel:window.QRCode.CorrectLevel.H});await new Promise(resolve=>setTimeout(resolve,60));const canvas=holder.querySelector("canvas");if(canvas)return canvas;const img=holder.querySelector("img");if(img){if(!img.complete)await img.decode();const c=document.createElement("canvas");c.width=300;c.height=300;c.getContext("2d").drawImage(img,0,0,300,300);return c}throw new Error("A QR-kód nem készült el.")}
async function drawTicketCanvas(order,code,index,total){
  ...
  ctx.fillStyle="rgba(255,255,255,.96)";ctx.beginPath();ctx.roundRect(60,210,1120,1400,34);ctx.fill();ctx.fillStyle="#12121a";ctx.font="900 55px Arial";let y=300;y=wrapCanvasText(ctx,order.eventName||"SIXTY NIGHT PARTY",105,y,1030,66,3)+30;ctx.fillStyle="#e1269c";ctx.font="900 34px Arial";y=wrapCanvasText(ctx,order.ticketName||"BELÉPŐJEGY",105,y,1030,43,2)+20;
  ctx.fillStyle="#383847";ctx.font="700 25px Arial";ctx.fillText(`Jegy ${index+1} / ${total}`,105,y);y+=55;ctx.fillStyle="#171722";ctx.font="800 28px Arial";ctx.fillText("IDŐPONT",105,y);ctx.fillText("HELYSZÍN",635,y);ctx.fillStyle="#4d4d5d";ctx.font="700 27px Arial";y+=43;wrapCanvasText(ctx,order.eventDate?huDate(order.eventDate):"A rendezvény adatlapja szerint",105,y,455,36,2);wrapCanvasText(ctx,order.eventLocation||"Sixty Night Party Park • Hatvan",635,y,455,36,2);y+=120;
  ctx.fillStyle="#171722";ctx.font="800 28px Arial";ctx.fillText("JEGY TULAJDONOSA",105,y);y+=44;ctx.fillStyle="#4d4d5d";ctx.font="700 34px Arial";wrapCanvasText(ctx,order.buyerName||order.billingName||"Vásárló",105,y,1000,44,2);y+=95;ctx.fillStyle="#171722";ctx.font="800 27px Arial";ctx.fillText("RENDELÉSI AZONOSÍTÓ",105,y);ctx.fillText("JEGYKÓD",635,y);y+=42;ctx.fillStyle="#4d4d5d";ctx.font="700 25px monospace";ctx.fillText(order.orderNumber||order.id,105,y);ctx.fillText(code,635,y);
  const qr=await makeQrCanvas(`${location.origin}/jegy-ellenorzes.html?code=${encodeURIComponent(code)}`);ctx.drawImage(qr,430,865,380,380);ctx.fillStyle="#171722";ctx.textAlign="center";ctx.font="800 27px Arial";ctx.fillText("BELÉPTETŐ QR-KÓD",620,1290);ctx.fillStyle="#666678";ctx.font="600 22px Arial";wrapCanvasText(ctx,"A QR-kód egyszeri belépésre jogosít. Ne oszd meg másokkal.",620,1332,850,32,3);
  ctx.textAlign="left";ctx.fillStyle="#171722";ctx.font="800 24px Arial";ctx.fillText(state.ticketShop.companyName||"Sixty Night Party",105,1460);ctx.fillStyle="#5c5c6b";ctx.font="600 20px Arial";let footerY=1498;footerY=wrapCanvasText(ctx,[state.ticketShop.companyAddress,state.ticketShop.taxNumber?`Adószám: ${state.ticketShop.taxNumber}`:"",state.ticketShop.contactEmail,state.ticketShop.phone].filter(Boolean).join(" • "),105,footerY,1020,28,4);ctx.fillStyle="#8a8a99";ctx.font="600 18px Arial";wrapCanvasText(ctx,"Ez a dokumentum belépőjegy. A hivatalos számla külön Billingo-bizonylatként kerül kiállításra.",105,footerY+20,1020,26,3);return canvas;
}
async function buildTicketPdf(order){
  if(!window.PDFLib||!window.QRCode)throw new Error("A PDF- vagy QR-kód modul nem töltődött be. Frissítsd az oldalt.");const codes=await ensureTicketCodes(order);const pdf=await window.PDFLib.PDFDocument.create();
  for(let i=0;i<codes.length;i++){const canvas=await drawTicketCanvas(order,codes[i],i,codes.length);const png=await pdf.embedPng(canvas.toDataURL("image/png"));const page=pdf.addPage([595.28,841.89]);page.drawImage(png,{x:0,y:0,width:595.28,height:841.89})}pdf.setTitle(`${order.orderNumber||order.id} – Sixty Night jegy`);return await pdf.save();
}
...
$("revokeOrderTickets").onclick=async()=>{const order=selectedOrder();if(!order)return toast("Válassz rendelést.","error");const codes=Array.isArray(order.ticketCodes)?order.ticketCodes:[];if(!codes.length)return toast("Ehhez a rendeléshez még nincs generált jegy.","error");if(!confirm("Biztosan érvényteleníted az összes jegyet?"))return;for(const code of codes)await setDoc(doc(db,"issuedTickets",code),{status:"revoked",revokedAt:serverTimestamp()},{merge:true});await updateSelectedOrder({status:"cancelled"},"A jegyek érvénytelenítve.")};
...
$("previewHomepage").onclick=async()=>{const previewWindow=window.open("about:blank","_blank");const ok=await hpPersist("draft");if(ok){const url=`/index.html?homepagePreview=1&v=670`;if(previewWindow)previewWindow.location.href=url;else window.open(url,"_blank")}else if(previewWindow)previewWindow.close()};
